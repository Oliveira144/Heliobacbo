...
# ========== FUNÇÕES ==========
def adicionar_rodada(player_dados, banker_dados):
    p_total = sum(player_dados)
    b_total = sum(banker_dados)
...
# ========== INTERFACE ==========
st.subheader("📥 Inserir nova rodada")
c1, c2 = st.columns(2)
with c1:
    player = st.text_input("🎲 Dados do Player (ex: 3,4)")
with c2:
    banker = st.text_input("🎲 Dados do Banker (ex: 5,6)")

if st.button("➕ Adicionar rodada"):
    try:
        p_dados = [int(x.strip()) for x in player.split(",") if x.strip()]
        b_dados = [int(x.strip()) for x in banker.split(",") if x.strip()]
        if len(p_dados) == 2 and len(b_dados) == 2:
            adicionar_rodada(p_dados, b_dados)
            st.success("Rodada adicionada com sucesso!")
        else:
            st.warning("Informe 2 dados para cada lado.")
    except:
        st.error("Entrada inválida. Use números como 3,4")
